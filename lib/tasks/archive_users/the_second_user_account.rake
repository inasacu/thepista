# to run:  heroku run rake the_new_user_email_account --app zurb
...
def update_user_switch(original_user_email, new_user_email)
    ...
    result = ActiveRecord::Base.connection.update_sql( "update authentications set user_id=#{the_original_id} where user_id=#{the_new_id}" )
