class UsersController < ApplicationController
  before_filter :require_no_user,     :only => [:signup, :create, :rpx_new, :rpx_create, :rpx_associate, :confirmation]
  ...
  def confirmation
    @user = User.find(:first, :conditions => {:confirmation_token => params[:token]})

    if (!params[:token].blank?) && @user && !@user.confirmation?
      @user.confirmation
      @user.save!
      successful_activation_message
      # redirect_to @user
    else
      reset_activation_message  
    end  
      redirect_to root_url
  end
