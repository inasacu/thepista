...
class Schedule < ActiveRecord::Base
  ...
  def self.user_active_events(user_id)
    begin
      ...
      logger.error("Exception while getting user active events #{exc.message}")
    ...
    begin
      ...
      logger.error("Exception while getting user groups events #{exc.message}")
      logger.error("#{exc.backtrace}")
      events_hash = nil
    end
	...
	def self.get_by_id(event_id)
      ...
      logger.error("Exception while getting event by id #{exc.message}")
      logger.error("#{exc.backtrace}")
      event = nil  
	  end
	  return event
