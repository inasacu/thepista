...
class Game < ActiveRecord::Base
	...
	def calculate_standing
		if self.cup.official
			...
			Game.set_final_stage(self.cup) if self.all_group_stage_played(self.cup).deliver unless USE_DELAYED_JOBS

			Standing.delay.cup_challenges_user_standing(self.cup) if USE_DELAYED_JOBS
			Standing.cup_challenges_user_standing(self.cup).deliver unless USE_DELAYED_JOBS
			Standing.delay.update_cup_challenge_item_ranking(self.cup) if USE_DELAYED_JOBS
			Standing.update_cup_challenge_item_ranking(self.cup).deliver unless USE_DELAYED_JOBS
		end	
		Standing.create_cup_escuadra_standing(self.cup)
		Standing.delay.calculate_cup_standing(self.cup) if USE_DELAYED_JOBS
		Standing.calculate_cup_standing(self.cup).deliver unless USE_DELAYED_JOBS
	end
	...
	def self.update_cast_details(cup)
		...
		cup.challenges.each do |challenge|       
  		...
  		Cast.update_cast_details(challenge).deliver unless USE_DELAYED_JOBS
			@cast = challenge.casts.first if @cast == ''
		end
		
		 unless (@cast == nil)
  		Cast.delay.calculate_standing(@cast) if USE_DELAYED_JOBS
  		Cast.calculate_standing(@cast).deliver unless USE_DELAYED_JOBS
		end
	end
	...
	def create_challenges_cast
		if self.cup.official
			self.cup.challenges.each do |challenge|
  			...
  			Cast.create_challenge_cast(challenge).deliver unless USE_DELAYED_JOBS
  				
  			if DISPLAY_PROFESSIONAL_SERVICES
				  Fee.delay.create_user_challenge_fees(challenge) if USE_DELAYED_JOBS
				  Fee.create_user_challenge_fees(challenge).deliver unless USE_DELAYED_JOBS
			  end
