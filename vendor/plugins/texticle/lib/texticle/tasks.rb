...
namespace :texticle do
  ...
  def ignore_multi_table(filename)
    the_path = Rails.root + 'app' + 'models'
    ignore_model = ['challenges_users','cups_escuadras', 'groups_markers', 'groups_roles', 'groups_users', 
      'roles_user', 'roles_users', 'user_mailer', 'user_session']

    the_filename = filename.gsub("#{the_path}/", '').gsub(".rb", '')
    return ignore_model.include?(the_filename)
  end
end
